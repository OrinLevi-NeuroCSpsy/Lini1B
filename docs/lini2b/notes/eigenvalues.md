...
אם $p_A(\lambda) = \det(A - \lambda I)$ הפולינום האופייני של $A$, אז:
$$p_A(A) = 0$$
...
**הגדרה:** מטריצה $A$ נקראת **משולשת** אם קיימת מטריצה הפיכה $P$ כך ש:
$$P^{-1}AP = T$$
כאשר $T$ מטריצה משולשת עליונה.
...
1. $m_A(x)$ מחלק את $p_A(x)$ (הפולינום האופייני)
2. ל-$m_A$ ול-$p_A$ אותם שורשים (אבל אולי ריבויים שונים)
3. $A$ לכסינה $\Leftrightarrow$ $m_A(x)$ מתפרק לגורמים לינאריים **שונים**
